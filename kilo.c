...
void enableRawMode(){
    ...
    atexit(disableRawMode); // при выходе возращаем изначальные атрибуты

    struct termios raw = E.orig_termios; // Создаем экземпляр структры для работы с ее флагом c_lflag (local flag)
    raw.c_iflag &= ~(BRKINT | ICRNL | INPCK | ISTRIP | IXON);
    raw.c_oflag &= ~(OPOST);
    raw.c_cflag |= (CS8);
    raw.c_lflag &= ~(ECHO | ICANON | IEXTEN |ISIG); // отключаем ECHO с помощью битовых операция НЕ и И и меняем таким образом флаг 
    raw.c_cc[VMIN] = 0;
    raw.c_cc[VTIME] = 1;
...
int getCursorPosition(int* row, int* cols){
    ...
    if(sscanf(&buf[2], "%d;%d", row, cols) != 2){
        return -1;
    }
    return 0;
}
